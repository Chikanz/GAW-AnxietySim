...
public class MatchSunRotation : MonoBehaviour
{
    ...
    void Update()
    {
       ...
       //Make the  sun move towards the player's gaze
       sun.rotation = Quaternion.Slerp(sun.rotation, Quaternion.LookRotation(-playerCamera.forward), speed * Time.deltaTime);
       
       //Clamp it so that it doesn't go below 10 on the x angle 
         if (sun.rotation.eulerAngles.x < 10)
         {
              sun.rotation = Quaternion.Euler(10, sun.rotation.eulerAngles.y, sun.rotation.eulerAngles.z);
         }
    }
}
